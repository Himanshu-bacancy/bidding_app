...
class Hcsubtopics extends BE_Controller {
	...
	function __construct() {
		...
		parent::__construct( MODULE_CONTROL, 'Hcsubtopics' );
	...
	function index() {
		...
		$this->data['rows_count'] = $this->Hcsubtopic->count_all_by( $conds );
		// get subtopics
		$this->data['hctopic'] = $this->Hcsubtopic->get_all_by( $conds , $this->pag['per_page'], $this->uri->segment( 4 ) );
		// load index logic
		parent::index();
	}
	...
	function add() {
		...
		// breadcrumb urls
		$this->data['action_title'] = get_msg( 'subtopic_add' );
	...
	function edit( $id ) {
		...
		// breadcrumb urls
		$this->data['action_title'] = get_msg( 'subtopic_edit' );

		// load user
		$this->data['topic'] = $this->Hcsubtopic->get_one( $id );
	...
	function save( $id = false ) {
        ...
        // prepare group_id
		if ( $this->has_data( 'topic_id' )) {
			$data['topic_id'] = $this->get_data( 'topic_id' );
		}
		...
		// save subtopics
		if ( ! $this->Hcsubtopic->save( $data, $id )) {
	...
	function delete( $id ) {
		...
		if ( !$this->ps_delete->delete_hcsubtopic( $id )) {
		...
		if ( !$this->check_trans()) {
			...
			$this->set_flash_msg( 'success', get_msg( 'success_subtopic_delete' ));
		}
	...
	{		
		 $conds['name'] = $name;
		 	...
		 	if ( strtolower( $this->Hcsubtopic->get_one( $id )->name ) == strtolower( $name )) {
			// if the name is existing name for that user id,
				return true;
			} else if ( $this->Hcsubtopic->exists( ($conds ))) {
	...
	/**
	 * Check Hcsubtopics name via ajax
	 *
	 ...
	 * Publish the record
	 *
	 * @param      integer  $id  The Hcsubtopics identifier
	 */
	function ajx_publish( $id = 0 )
	{
		...
		// save data
		if ( $this->Hcsubtopic->save( $topic_data, $id )) {
			echo 'true';
		} else {
			echo 'false';
		}
