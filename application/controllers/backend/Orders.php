...
//		$this->data['item'] = $this->Item->get_one( $item_id );
    ...
    public function returndetail($id) {
        ...
        // load order
        //$order = $this->db->query("SELECT * FROM `bs_order` WHERE id = '".$id."'")->result();
        $return_details = $this->db->select('bs_return_order.id,bs_return_order.order_id,bs_reasons.name as reason_name,bs_return_order.description,bs_return_order.status,bs_return_order.created_at,bs_return_order.seller_response, bs_order.user_id, bs_order.items')
